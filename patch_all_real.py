...
arXiv:1801.04883) describes, rather than with its visible hyperparameters only.
  ...
  7. requires_grad fix   set_requires_grad([netD_A, netD_B], False) walks
                         netD_*.parameters(), which under --share_embedding now
                         contains W_Emb. Without this the shared table is frozen
                         for the whole generator step and receives the
                         adversarial objective only, never the cycle objective.

CHANGES FROM THE PREVIOUS VERSION
  - Section 4 anchored on `_dp = itertools.chain(...)`, a form that never
    existed in the repository. It now anchors on the real optimizer_D block and
    still recognises the `_dp` form if a previous run produced it.
  - Section 7 is new.
"""
...
if "_unique_params(self.netG_A" not in s:
    ...
    if old_g not in s:
        die("optimizer_G", "sed -n '145,165p' models/cipher_cycle_gan_model.py")
    s = s.replace(old_g, """            self.optimizer_G = torch.optim.Adam(
                _unique_params(self.netG_A.parameters(),
                               self.netG_B.parameters()),""", 1)
    done.append("optimiser_G deduplication")

if "_unique_params(*_dp)" not in s:
    new_d = """            _dp = [self.netD_A.parameters(), self.netD_B.parameters()]
            if getattr(opt, "share_embedding", False):
    ...
    # form actually present in the repository
    plain_d = """            self.optimizer_D = torch.optim.Adam(
                itertools.chain(self.netD_A.parameters(),
                                self.netD_B.parameters()),"""
    # form a previous partial run may have left behind
    chain_d = """            _dp = itertools.chain(self.netD_A.parameters(),
                                  self.netD_B.parameters())
            self.optimizer_D = torch.optim.Adam(
                _dp,"""

    if plain_d in s:
        s = s.replace(plain_d, new_d, 1)
    elif chain_d in s:
        s = s.replace(chain_d, new_d, 1)
    else:
        die("optimizer_D", "sed -n '145,165p' models/cipher_cycle_gan_model.py")
    done.append("optimiser_D deduplication + embedding in D")
...
# ------------------------------------------- 7. keep W_Emb live for the G step ---
if "keep the shared table trainable" not in s:
    a = "        self.set_requires_grad([self.netD_A, self.netD_B], False)"
    if a not in s:
        die("set_requires_grad False",
            "grep -n 'set_requires_grad' models/cipher_cycle_gan_model.py")
    s = s.replace(a, a + '''
        if getattr(self.opt, "share_embedding", False):
            ...
            self.set_requires_grad(
                [(self.netG_A.module if hasattr(self.netG_A, "module")
                  else self.netG_A).embedding], True)''', 1)
    done.append("shared embedding stays trainable during the G step")
