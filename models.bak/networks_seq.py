...
class ResBlock1d(nn.Module):
    ...
    def __init__(self, ch, dilation, norm_layer):
        super().__init__()
        pad = dilation
...
class SeqGenerator(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim=64, ngf=128, n_blocks=4,
        ...
        self.body = nn.Sequential(*([] if pointwise else [
...
class SeqDiscriminator(nn.Module):
    ...
    def __init__(self, vocab_size, embed_dim=64, ndf=128, n_layers=3,
                 norm_layer=nn.InstanceNorm1d):
        super().__init__()
        self.embedding = SoftEmbedding(vocab_size, embed_dim)
        layers = [nn.Conv1d(embed_dim, ndf, 4, stride=2, padding=1),
                  nn.LeakyReLU(0.2, inplace=True)]
        mult = 1
        for n in range(1, n_layers):
            prev, mult = mult, min(2 ** n, 8)
            layers += [
                nn.Conv1d(ndf * prev, ndf * mult, 4, stride=2, padding=1),
                norm_layer(ndf * mult),
                nn.LeakyReLU(0.2, inplace=True),
            ]
        layers += [nn.Conv1d(ndf * mult, 1, 4, stride=1, padding=1)]
